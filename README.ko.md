...
입구는 목적·입력·결과와 기능들의 연결을 큰 관계도로 보여 줍니다. 준비된 기능을 누르면 데이터 흐름으로, 처리 노드를 누르면 해당 규칙의 그림으로 들어갑니다. 구현 위치·관련 연결·테스트 근거는 필요할 때 펼칩니다. 기능 목록·검색·폴더 탐색도 유지하며, 돌아오면 선택한 기능이나 노드를 다시 보여 줍니다. 중요한 분석 한계는 먼저 표시합니다. 테스트 위치가 테스트 통과나 전체 변경 영향 분석을 뜻하지는 않습니다. 보관한 원본으로 다시 빌드하면 새 UI를 적용할 수 있습니다.
