...
M5에서는 **책임 구역·대표 기능 목록·선택한 상세 설명 생성·지도↔동작↔규칙 이동**을 추가했습니다. 돌아오면 지도 선택과 확대·이동 위치가 복원됩니다. `npm run eval:atlas -- --output build/eval/m5` 후 `build/eval/m5/agent-project.html`에서 확인합니다. 기존 동작 24개·규칙 6개에 지도 6개를 추가했습니다. [프로젝트 지도 사용 안내](docs/project-maps.md) · [M5 검증 기록](eval/reports/m5-verification.md).

**[GitHub에서 최종 화면 미리보기·HTML 받기](eval/m5/README.md)** — 지도 6개와 연결된 동작·규칙 페이지를 함께 보관했습니다.
...
프로젝트 지도에는 같은 체크아웃의 `codebase-atlas`도 설치합니다: `npx skills add . --skill codebase-atlas`. code-flow는 공통 렌더러로 함께 필요하며, 규칙 설명까지 요청할 때 visual-primer를 사용합니다. `$codebase-atlas 이 프로젝트의 목적과 주요 구성, 대표 기능을 설명해 줘.`로 시작합니다.
...
├── README.md
├── README.ko.md
├── skills/codebase-atlas/         # 프로젝트 탐색, 기능 목록, 연결 페이지 조립
