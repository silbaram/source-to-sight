...
For regeneration, read `s2s.py inspect <existing.html>` and the retained internal graph, then check the current source. Presentation-only re-renders can reuse reviewed facts through the normal build checks; changed source, scope or claims need renewed discovery/review. For map-linked pages, reuse the owning map's `_internal/<map-html-stem>/` bundle and its atlas assembly workflow; keep original rule layouts there too. Do not merely change hashes or timestamps on stale assertions. The offline page does not execute a generation command when clicked.

For a requested rules explanation (`--explain` in a skill prompt), read [rule-checklist.md](references/rule-checklist.md), then discover the installed `visual-primer/SKILL.md` and its source-rules mode. Use `author.py explain` to start a pending review from the internal behavior graph, preserve the subject/scope, and reread source before adding reasons or exceptions. The final drill-down is a topic-specific picture lesson about the important business rules, not another fixed workflow chart or a collection of rule cards: use visual-primer's authored layout by default, including a supplied visual reference. If the companion is absent, finish the authorized behavior page and state that the additional rules skill is unavailable. Locate sibling skills through the host's skill catalog or an actual sibling directory, never a hardcoded user path. Link only generated, identity-checked pages; otherwise provide the precise generation request. Installation and initial rendering work with `code-flow` alone.
