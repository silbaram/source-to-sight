...
For the rules page, locate the available `visual-primer` skill and read its source-rules guide. Its composition chooses condition/result, comparison and state figures in an order suited to the subject. Do not add inter-rule execution arrows or automatic simulations merely because rules appear next to one another. A missing companion does not prevent `code-flow` from producing the authorized behavior page.
