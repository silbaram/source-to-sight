...
Playback frames the current connection, shrinking only when needed to fit the available map area. It runs each step for 1.5 seconds, including the final step; a one-step scenario can play once. Pause, manual selection, scenario/view changes, and a hidden page cancel the marker. Rebuilding SVG on resize keeps the current step's elapsed progress, and switching reduced-motion preferences updates the moving/static presentation immediately. Reduced motion retains arrows, endpoint names, evidence status, and timed step progression without moving markers or arrival pulses.
