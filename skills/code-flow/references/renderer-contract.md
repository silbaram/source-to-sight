...
M4 provides 1×, 1.5× and 2× speeds and a direct step selector in the walkthrough player. The speed applies to a shared explanation clock used by the step timeout, progress and moving marker. Changing it preserves elapsed time during playback, pause or camera movement; it neither restarts the step nor advances it while the camera is framing. Direct selection pauses playback, updates the caption/condition/evidence and exposes required detail nodes. No-scenario pages keep these controls hidden.
