...
Atlas pages default to `view=overview`: a short orientation, role cards, optional reviewed package/folder landmarks, and a capability catalog with search and region/detail-availability filters. The full-diagram control and old `view=structure`/`view=flow` links preserve the existing canvas. This default applies only to atlas pages; behavior and authored rule pages are unchanged. Missing catalog/structure data produces explicit empty states, not synthetic features or folders.
...
Overview return state uses the versioned hash with optional `tab` (roles/files), `q` (up to 240 characters), `group` (region ID), `available` (ready/missing), and `feature` (capability ID), without camera or diagram-selection fields. These fields are rejected on other views. Returning through behavior/rules restores the tab, filters and selected feature; folder disclosures themselves are local. Diagram keyboard shortcuts are inactive on the overview. Both entry views use safe text DOM, existing theme variables and no network fetches.

When a restored feature is present in the filtered catalog, focus and immediately bring its card into the viewport; selection alone must not leave it below the visible page. Folder sorting uses the recorded language when the browser accepts it, otherwise a deterministic string order. Unsupported browser locale tags must not interrupt initialization or rewrite the IR language.
