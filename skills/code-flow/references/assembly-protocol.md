...
Keep map-linked authoring files in the owning map's `<HTML directory>/_internal/<map-html-stem>/`, unless the user or repository specifies another private directory. Atlas builds retain the complete graphs/layouts and merge their paths into `pages.json`. For adding or refreshing only one capability, use a separate requested-pages manifest so the saved inventory is not replaced with a subset. The bundle is reusable input, not shareable output; exclude it from HTML packages and static-site publishing. Standalone `author.py build` does not automatically create an atlas bundle.
