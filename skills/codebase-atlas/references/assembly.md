...
The map checks catalog target/scope/language and the source snapshot. A missing, incompatible or stale detail remains a generation request. A matching generated page is a link; pages that were not requested are neither generated nor rewritten. Rebuild the map after generating another child to refresh its availability.

Map-to-detail navigation carries the current region, selection and camera as URL state; returning uses the atlas URL recorded in the detail, never a supplied external destination. Browser storage is not required. These local links require the HTML files to be available and do not publish them.
