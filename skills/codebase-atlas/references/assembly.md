...
The entry is a quick orientation surface, not a fixed number of document sections. It shows the project purpose and recorded inputs/results, then capabilities grouped by their actual responsibility region (or owner node when ungrouped). There is no mandatory web-app hierarchy or fixed card count. The purpose should state who the project serves and what it produces; keep input/output labels short. Explain essential runtime, deployment form or external boundaries in reviewed purpose/component text when relevant. Never infer a language version, service topology or dependency from a directory suffix.

Feature cards show a short summary, owner, one associated non-root path when available, their own review status and detail-page availability. They do not claim that unrecorded capabilities are absent. Components without catalog entries remain in a supporting band; capability owners are not repeated as a separate role catalog. The complete folder tree is available on demand. Existing `view=structure` and `view=flow` links still open the full diagram; behavior and picture-rule templates keep their roles.

Each group is a full-width, bordered/tinted container with its own title, recorded capability count and Group diagram control. Member cards wrap inside it; groups stack so unequal sizes do not create empty columns. Filtered counts distinguish visible from recorded capabilities. Group membership means shared responsibility, not a required sequence or a claim that the capabilities must be combined.

Selecting a capability opens a wide summary dialog (full-screen on small phones), with no permanent side pane. It shows the capability's own status, owner, a representative path, direct relationships and rule summaries. Other paths, source evidence, related flows, rule conditions/reasons/exceptions, tests and scope open on demand; these are not removed from the data. The fixed actions reach the existing precise detail page or diagram. Connections are not an exhaustive change-impact analysis. No feature or dialog opens automatically. Dismissal preserves selection, filters and page scroll, returning focus to the invoking card. Choosing a capability from search or a tree item's capability action opens its dialog; restored detail links highlight/focus the card without reopening it. Dialog state is local, not a new URL field. Close the dialog before opening search, a tree path or the diagram; clipboard fallback must remain usable inside it.
