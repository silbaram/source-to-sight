...
Help an AI-assisted maintainer quickly identify what a repository does, which capabilities exist, how responsibilities fit together, and where to start exploring a change. Lead with a short purpose and input/output overview, then a dominant diagram of responsibilities and their actual connections. Follow a component into its capability data flow, then a selected node into its picture-first rule lesson. Keep the searchable capability catalog and maintenance evidence on demand. Keep real names/paths secondary and material analysis limits visible. Read the actual source and documents; the tools validate and render reviewed findings rather than infer arbitrary architecture.
