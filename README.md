...
Project maps now open with a short overview, responsibility cards, reviewed package/folder landmarks, and a searchable capability catalog. The full interactive structure/flow diagram remains one click away, and existing diagram deep links still work. Folder nesting is containment, not an inferred dependency. Older JSON without package landmarks stays readable with an explicit unrecorded-structure message; re-render existing HTML to receive the new UI.
