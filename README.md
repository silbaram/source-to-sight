...
Project maps show a visual composition of the actual applications, packages, execution environments, storage and external connections first. Below it, each real capability has its own incoming-request/result header above collaborating processing boxes and labeled transfers. A payment feature might contain controller, service and database responsibilities; these are parts of that feature, not separate capability groups. Each preview reuses the exact checked behavior graph, preserving alternate/error paths and parallel boundaries. A processing box opens its matching node explanation and rule pictures; return links restore the feature. Search, folders, full structure and evidence remain available on demand. Missing details stay explicit. Add reviewed composition references to retained inputs and rebuild to use this overview; old area-based inputs remain readable.
