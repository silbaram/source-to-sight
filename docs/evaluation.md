...
평가 도구는 소스에서 작성한 설명 후보를 고정 기준과 비교합니다. 현재 6개 공개 저장소·3개 언어·6유형의 동작 후보 24개와, 같은 대상의 규칙 후보 6개와 프로젝트 지도 6개가 있습니다. 코드 위치·관계·필수 행동 주장과 경로·불확실성을 자동으로 검사하고, 문장의 사실성·규칙·이해도는 사람이 검토합니다. 규칙 평가는 원래 동작의 사실·근거가 보존되는지와 조건 비교 그림에 필요한 규칙이 있는지도 확인합니다.
...
S2S_EVAL_DIR=build/eval/m5 npx playwright test tests/evaluation.spec.cjs tests/m2.spec.cjs tests/m3.spec.cjs tests/m4.spec.cjs tests/m5.spec.cjs
~~~

출력 디렉터리는 새 경로여야 합니다. 다시 실행할 때 `build/eval/m5-second`처럼 이름을 바꾸면 이전 결과와 답변이 보존됩니다. 저장소 소스는 실행하지 않으며 모델 API도 호출하지 않습니다. 고정된 소스를 읽어 현재 캔버스·규칙 템플릿으로 HTML을 만듭니다. M5는 기존 30개 동작·규칙 후보를 그대로 사용하고 여섯 유형의 프로젝트 지도를 추가합니다. 각 지도에는 명시적으로 선택한 동작·규칙 쌍만 조립합니다. 유틸리티 지도는 입력 계약 규칙 설명을 함께 검증하므로 총 HTML 수는 37개이며, 이를 별도 독립 생성 사례로 세지 않습니다. 동작 24개만 실행할 때는 `npm run eval:run -- --output <새 경로>`를 사용합니다.
...
| `behavior-results.json` | 기존 동작 24개 결과 |
| `rules-results.json` | 지도 조립 전에 보존한 기존 동작·규칙 30개 결과 |
| `<case-id>.review-template.json` | 답을 채우지 않은 사람 검토 양식 |

자동 통과와 최종 승인을 구분합니다. 검토가 없으면 `pending-review`이며 기본 종료 코드는 0입니다. `--strict`를 추가하면 선택한 전체 묶음(`eval:run`은 24개, `eval:rules`는 30개, `eval:atlas`는 36개)이 자동 검사와 사람 검토를 모두 통과해야 0입니다. 일부 사례 실행은 전체 승인으로 처리하지 않습니다.
...
18개에서 24개, 24개에서 30개 또는 30개에서 36개로 확대한 결과를 비교할 때는 `--allow-suite-extension`을 명시합니다. 두 실행 모두 각 manifest의 전체 사례를 포함해야 하며 기존 사례의 정답·분류와 소스 목록은 같아야 합니다. 비교 결과에는 기존 비교 수와 추가 사례 목록이 나옵니다. 기존 누락과 신규 실패를 무시하는 옵션이 아닙니다.
...
`eval:run`에서는 `--case utility-strip --case plugin-hooks`처럼 반복 지정해 일부만 실행할 수도 있습니다. 공통 지침 변경은 유형별 최소 1개와 과거 실패 사례, 특정 프로파일 변경은 해당 4개와 다른 유형 최소 3개, 공통 렌더러·스키마·검증기 변경은 `eval:atlas`로 동작 24개·규칙 6개·지도 6개를 모두 실행합니다. 도구가 변경 파일로 이 집합을 자동 선택하지는 않습니다.
...
M5의 [지도 manifest](../eval/atlas-cases.json)는 지도·동작·규칙·그림 구성과 정답을 지정합니다. 지도 검사는 소스 역할과 관계, 책임 구역 소속, 기능의 대상·범위·담당 역할 및 세 계층 링크를 확인합니다. 지도 후보 해시는 공급한 동작·규칙·그림 구성도 포함합니다. 목적 설명과 묶음의 이해 가능성은 사람 검토 항목입니다. [소스 읽기 기록](../eval/m5/discovery.md)을 함께 확인합니다.
...
npm run eval:atlas -- --reviews build/reviews --output build/eval/atlas-reviewed --strict
~~~

후보·정답·세 스킬의 지침/렌더러/평가 코드/루브릭이 바뀌면 기존 답변은 오래된 검토로 판정합니다. M3는 그림 구성 또는 원본 동작만 바뀌어도 재검토합니다. 외부 동작 후보를 검토했다면 재실행에도 같은 `--candidates`를 지정합니다. 규칙 후보는 검토한 규칙 manifest를 유지합니다. 소스 위치가 맞다는 것만으로 문장·수치가 정확하다고 승인하지 않습니다.
...
M5는 `eval/m5/author_cases.py`로 지도 후보를 기록하고 `reference_cases.py`에 별도의 소스 역할·관계·구역·기능 담당 기준을 유지합니다. 기존 정답 파일은 덮어쓰지 않습니다. 기능 ID와 범위는 처음 고정한 계약이며, 후보를 바꿨다는 이유로 정답을 맞춰 재생성하지 않습니다.
