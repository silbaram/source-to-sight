...
`visual-primer`는 개념 설명과 M3의 소스 기반 규칙 설명을 제공합니다. `$code-flow --explain`은 기존 동작의 사실을 재사용해 조건·결과·이유·예외를 비교하는 페이지를 만들고 두 화면을 연결합니다. `codebase-atlas`의 프로젝트 전체 탐색은 M5 개발 범위입니다. 현재 구현의 독립적인 사람 검토는 대기 중입니다.

| 안내 | 내용 |
| --- | --- |
| [한국어 시작 안내](../README.ko.md) / [English](../README.md) | 설치, 스킬 사용, 실제 사례와 지원 범위 |
| [개발 가이드](development.md) | 개발 환경, 예제 재현, 템플릿 변경과 검사 방법 |
| [평가 가이드](evaluation.md) | 동작 24개·규칙 6개 실행, 결과 비교와 사람 검토 반영 |
