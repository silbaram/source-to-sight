...
`visual-primer`는 개념 설명과 M3의 소스 기반 규칙 설명을 제공합니다. `$code-flow --explain`은 기존 동작의 사실을 재사용해 조건·결과·이유·예외를 비교하는 페이지를 만들고 두 화면을 연결합니다. `codebase-atlas`는 프로젝트 목적·책임 구역·대표 기능을 지도로 만들고 필요한 동작·규칙 설명을 연결합니다. 미생성 기능에는 정확한 범위의 생성 요청을 제공하며, 지도 복귀 시 선택과 화면 위치를 복원합니다. 현재 구현의 독립적인 사람 검토는 대기 중입니다.

M4의 동작 화면에서는 배속·단계 선택·키보드 탐색과 모바일 설명 패널 크기 조절을 사용할 수 있습니다. 현재 위치 링크를 복사하면 같은 HTML의 노드나 흐름 단계를 다시 열 수 있으며 자동재생은 시작하지 않습니다.
